...
if jq empty "$OUTCOME" 2>/dev/null; then
...
assert_eq "outcome has schema_version" "true" \
  "$(jq 'has("schema_version")' "$OUTCOME" 2>/dev/null || echo false)"

# Has outcome_types array with 5 entries
assert_eq "outcome has 5 outcome_types" "5" \
  "$(jq '.outcome_types | length' "$OUTCOME" 2>/dev/null || echo 0)"
...
for otype in full_pass partial_pass agent_failure infra_failure timeout; do
  assert_eq "outcome_types contains $otype" "true" \
    "$(jq --arg t "$otype" '[.outcome_types[].name] | index($t) != null' "$OUTCOME" 2>/dev/null || echo false)"
done
...
# File exists and is valid JSON
if jq empty "$FEEDBACK" 2>/dev/null; then
...
assert_eq "feedback has schema_version" "true" \
  "$(jq 'has("schema_version")' "$FEEDBACK" 2>/dev/null || echo false)"

# Has all required fields defined
for field in bead timestamp template agent model outcome signals failure_patterns prompt_hash; do
  assert_eq "feedback defines field '$field'" "true" \
    "$(jq --arg f "$field" '.fields | has($f)' "$FEEDBACK" 2>/dev/null || echo false)"
done
...
for sig in exit_clean tests_pass lint_pass ubs_clean truthsayer_clean duration_ratio retried; do
  assert_eq "feedback signals has '$sig'" "true" \
    "$(jq --arg s "$sig" '.fields.signals.fields | has($s)' "$FEEDBACK" 2>/dev/null || echo false)"
done

# outcome field references the outcome types
assert_eq "feedback outcome references outcome_types" "true" \
  "$(jq '.fields.outcome.enum != null' "$FEEDBACK" 2>/dev/null || echo false)"
